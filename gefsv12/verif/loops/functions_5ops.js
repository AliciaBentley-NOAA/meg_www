	...
	if(mode == 'valid'){
		...
		for(i=0; i<variables.length; i++){
			...
			element.add(option);
                ...
                var element = document.getElementById("parm");
                for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}

                for(i=0; i<parms.length; i++){
                        var option = document.createElement("option");
                        option.text = parms[i].displayName;
                        option.value = parms[i].name;
                        element.add(option);
                }
        }
...
function getURL(domain,parm,variable,frame,level,season,validtime){
	var newurl = url.replace("VVV",variable);
	newurl = newurl.replace("LLL",level);
        newurl = newurl.replace("PPP",parm);
...
function showImage(){
	...
	document.getElementById("domain").selectedIndex = searchByName(imageObj.domain,domains);
        ...
        document.getElementById("parm").selectedIndex = searchByName(imageObj.parm,parms);
...
function preload(obj){
	...
	for (i2=0; i2<hrs_loop.length; i2++){
		...
		var urls = getURL(obj.domain,obj.variable,i,obj.level,obj.season,obj.parm);
...
//Change the map parm from dropdown menu
function changeParm(id){
        imageObj.parm = id;
        preload(imageObj);
        showImage();
        document.getElementById("parm").blur();
}
