...
function populateMenu(mode){
	if(mode == 'valid'){
		...
		for(i=minFrame; i<=maxFrame; i=i+incrementFrame){
			...
			var increment = (i*24) - (startFrame*24);
			option.text = formatDate(increment,'valid');
			option.value = i;
			element.add(option);
		}
	}
...
function updateMobile(){
	...
	|| navigator.userAgent.match(/Windows Phone/i)
		...
		document.getElementById('page-middle').innerHTML = "Swipe Up/Down = Change model version | Swipe Left/Right = Change initialization time";
	}
